...
var sizeLabel = document.querySelector("#pizzaSize");
...
// With the current implementation, randomPizzaContainer's offsetWidth could be one of three values.
function getRandomPizzaContainerOffsetWidth(size) {
    switch (size) {
        case "1":
            return 293;
        case "2":
        ...
        case "3":
            return 585;
        default:
            console.log("bug in getRandomPizzaContainerOffsetWidth");
    }
}

// Returns the size difference to change a pizza element from one size to another. Called by changePizzaSlices(size).
function determineDx(pizzaOffsetWidth, size) {
    var oldWidth = pizzaOffsetWidth;
...
var resizePizzas = function(size) {
    ...
    var currentRandomPizzaContainerOffset = getRandomPizzaContainerOffsetWidth(size);

    // Iterates through pizza elements on the page and changes their widths
    for (var i = 0; i < randomPizzas.length; i++) {
        var dx = determineDx(currentRandomPizzaContainerOffset, size);
        var newwidth = (currentRandomPizzaContainerOffset + dx) + "px";
